...
void UOdinRoom::BeginDestroy()
{
    ...
    if(submix_listener_.IsValid())
        submix_listener_->StopSubmixListener(submix_listener_);
}
...
void UOdinRoom::UpdateAPMConfig(FOdinApmSettings apm_config)
{
    ...
    if (odin_apm_config.echo_canceller) {
        if (!submix_listener_.IsValid()) {
            submix_listener_ = MakeShared<FOdinSubmixListener>();
            submix_listener_->SetRoom(this->room_handle_);
        }
        submix_listener_->StartSubmixListener(submix_listener_);
